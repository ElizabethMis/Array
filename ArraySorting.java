...
public class ArraySorting {
	public static void main(String[] args) {
        ...
        for(int currentIndex = 0; currentIndex < numbers.length; currentIndex++){
			int smallestValue = numbers[currentIndex];
			int smallestIndex = currentIndex;
		  for(int ix = currentIndex; ix < numbers.length; ix++) {
	    	  if(numbers[ix] > smallestValue) {
			  } else if(numbers[ix] < smallestValue) {
				  smallestValue = numbers[ix];
				  smallestIndex = ix;
			  } else { 
			  }
			  System.out.println(numbers[currentIndex]+numbers[1]);	
		  }	
		  int temp = numbers[currentIndex];
		  numbers[currentIndex] = numbers[smallestIndex];
